# https://github.com/casey-meehan/data-copying/blob/master/data_copying_tests.py
...
def Zu_cells(Pn, Pn_cells, Qm, Qm_cells, T, T_cells, s=0.05): 
    ...
    k = len(np.union1d(np.union1d(np.unique(Pn_cells), np.unique(Qm_cells)), np.unique(T_cells)))
    Zu_cells = np.zeros(k)
    ...
    for i in range(k): 
        ...
        # s, p = shapiro(Pn_cell_i)
        # if p < 0.05:
        #     print(f'WARNING Pn fails normality in cell {i} with statistic={s}')

        # s, p = shapiro(T_cell_i)
        # if p < 0.05:
        #     print(f'WARNING T  fails normality in cell {i} with statistic={s}')
        ...
        if len(Qm_cell_i) > 0:
            ...
            # s, p = shapiro(Qm_cell_i)
            # if p < 0.05:
            #     print(f'WARNING Qm fails normality in cell {i} with statistic={s}')
...
def C_T(Pn, Pn_cells, Qm, Qm_cells, T, T_cells, tau):
    ...
    n = Pn.shape[0]
    if n < 20 or m < 20:
        print(f'WARNING | n={n} | m = {m}')
    k = np.max(np.union1d(np.union1d(np.unique(Pn_cells), np.unique(Qm_cells)), np.unique(T_cells))) + 1 #number of cells
